...
**Vertical rhythm (2026-09):** the routed page wrapper (`App.jsx` → `.pageShell`) owns it — **200px between every section** (80 on phones) and 200px before the footer. Pages set only their horizontal gutters and internal spacing, never section-to-section gaps. The **header is fixed/floating**, so pages clear it via `--header-clearance` (bar bottom + 200) on the shell — never a per-page top padding. The front-page hero is **content-sized** (no fixed height), so its air above/below matches that 200.
