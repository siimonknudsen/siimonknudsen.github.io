...
**Conventions (don't undo):** **Tight radius** — cards/panels/images/buttons **≤8px** (the `md`–`3xl` tier is 8px in `index.css`; `sm` 4px micro; `pill` for circular: avatars, chips, the nav bar). *(A brief 0px "sharp corners" experiment on 2026-08 was reverted — keep the 8px rounding.)* **Flat B&W direction** (2026-08): backgrounds are neutral (near-black `#0a0a0a` / white). **Monochrome palette (2026-09):** there is no brand hue — `--accent` is the max-contrast ink (white on dark, black on light); only status colours (the green availability dot, feedback roles) carry hue. **Content cards, chips & logo tiles are FLAT SOLID tonal surfaces, NOT glass** — testimonial / skill / project / impact cards + logo tiles use `--surface-color-secondary`; chips (`Badge`) use `--surface-color-tertiary`; all with **no border, no shadow, no hover glow** (the cursor-spotlight `.fx-spotlight` is retired site-wide). **Glass is reserved for floating chrome only** — the header bar + its dropdowns, tooltips, modals — and there it carries **no outer hairline border and no inset top-edge highlight**, just a **small subtle drop-shadow** (`--glass-shadow-lg` = `0 4px 16px`). The **header bar and its dropdowns share the exact same glass** (`--glass-bg` tint + 20px blur): dark ≈ `#1c1c1e`, light = **white @ 60%**. Keep glass on the `glass`/`glass-panel` classes (semantic tokens, never hardcoded). Nav triggers are **text-only** (colour shift, no pill). **Nested radii are concentric** (inner < outer — e.g. a project-card image is `sm`/4px inside the card's `xl`/8px). The testimonial quote glyph is **greyscale** (`--text-color-tertiary`), not accent. Page-transition wipe = neutral grey `--neutral-500`, portaled to `<body>`. **Same-page anchors never page-switch:** an anchor/hash link to a section on the *current* page must just **smooth-scroll** (via `ScrollToTop`) — it must **not** trigger the curtain wipe / `page-enter-nav`. The wipe plays **only on a real `pathname` change** (`App.jsx` `AnimatedRoutes` compares `location.pathname` to the previous one; hash-only navs are ignored).

**Vertical rhythm (2026-09):** the routed page wrapper (`App.jsx` → `.pageShell`) owns it — **128px between every section** (64 on phones) and 128px before the footer. Pages set only their horizontal gutters and internal spacing, never section-to-section gaps. The **header is fixed/floating**, so pages clear it via `--header-clearance` (bar bottom + 128) on the shell — never a per-page top padding. The front-page hero is **content-sized** (no fixed height), so its air above/below matches that 128.

**Repo map:** pages → `src/pages/` · components → `src/components/` (`home/`, `projects/`,
`buttons/`, `cards/`, `grids/`, `motion/`) · project list → `src/data/projects.js` · per-project
content → `public/projects/<id>/content.json` · tokens + globals → `src/index.css` · shell +
...
black / white / grey — no brand hue, few weights. **Motion is calm, slow, relaxing** — never
