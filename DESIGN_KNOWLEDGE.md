...
- Imagery/video style:
  ...
  - **Monochrome gradients (2026-09-05):** Simon asked for "beautiful gradients / gradients from images" within the monochrome direction — but every gradient execution offered that day (top-edge glows, gradient hairlines, desaturated image washes, ink-fill type) was rejected along with the variants carrying them (see §6.8). Treat the appetite as real but unproven: if gradients-as-light return, start from ONE reference he supplies, not from technique research.
...
| **Warm "ember" mesh background** (orange/peach shader + radial wash over the whole page, both themes) | **Clean black & white** field — "less is more" | The warm atmospheric wash competed with the content and read as "too much." A **neutral field makes the single orange accent pop harder** — the "restrained neutral + one warm accent" DNA is *sharpened* by removing background colour, not weakened. Keep backgrounds achromatic (near-black / white, flat solid field — no texture); spend the colour budget on the accent, not the canvas. Depth still comes from glass + type + whitespace, not a coloured backdrop |
| **1200ms circular clip-path theme reveal** (View Transitions "ink bloom" from the toggle) | **Calm ~320ms full-page colour cross-fade** | The big geometric wipe is the most-copied portfolio theme effect and drew attention to itself — decorative + attention-grabbing, against the DNA. A snapshot dissolve reads "instant but buttery." Theme switch is plumbing, not a showpiece — keep it quiet |
...
| **The experience-section variant saga (2026-09-05)** — THREE rounds × 5 variants: quiet table refinements (year index, statement rows, tenure bars) → accordions/cards/gradient panels → award-tier interactions (Snellenberg cursor-preview index, scroll-scrubbed years, ink-fill type, filmstrips, full-bleed chapters). All 15 deleted. | After round 1 he said **"1 is the cleanest"** (the ledger — his existing table + micro-craft); after round 2 "so bad and boring"; after round 3 "no design taste… let's choose what we started with." Shipped: the original table with only the ledger's micro-craft (primary-ink company, tabular figures, tuned columns, hover wash, "Now"). | **Converge on his first pick — don't keep generating species.** The signal was there after round 1. (a) "Boring" did NOT mean "go bigger": swinging to display-type showpieces made it worse — his own §6.7/§6.8 DNA (never templated, anti-gimmick) predicted that rejection, and borrowed award-site patterns (Snellenberg/Cuberto) read as *templates* to him precisely because everyone copies them. (b) When direction is unclear after a rejection, ask him for ONE reference he loves before building anything. (c) Variant explorations: one round max, then iterate in small steps on whatever he anchored to. (d) Trust the documented taste model over a frustrated message — re-read §6.7/6.8 before overriding it. |
...
- **Rebalance a top-heavy webfont at the source (`ascent/descent/line-gap-override` on the *primary* face)** — some typefaces ship a lopsided box: the line-box centre sits off the caps' optical centre, so **every** flex-/grid-centred label (and text generally) rides high or low — site-wide, not per-component. Two fixes: per-element `text-box-trim` (above), or **rewrite the metrics on the `@font-face`** so it's balanced for all uses at once. The cap-centring rule is **`ascent-override − descent-override = cap-height`** (all % of em) → the content box then centres caps in *any* line-box. Hold **`descent-override ≥ the descender ink`** or g/j/p/y clip; tune **`line-gap-override`** to keep `line-height: normal` where it was. **The non-obvious gotcha:** the content box `ascent+descent` is bounded *below* by `cap + 2·descender`, so any **multi-line** run whose `line-height` is under that **overlaps** — you must floor tight **display** leading to clear it (e.g. balanced **BDO Grotesk**: native asc 800 / desc 177 / cap 729 per 1000 em → overrides 90.6 / 17.7 / 29.4% → 1.083em box → display line-heights floored to **1.1**; caps then centre to ~0.1px). Measure live, don't eyeball: canvas `ctx.measureText` → `fontBoundingBoxAscent/Descent`, `actualBoundingBoxAscent('H')` = cap, `actualBoundingBoxDescent('gjpy')` = descender ink. *(Same override CSS as the fallback-matching trick above — but aimed at the **real** font for optical balance, not CLS. `text-box-trim` is the lighter touch — no rhythm shift, no display-floor — pick it unless you specifically want one global mechanism. Verified here 2026-06-07; see DESIGN_LOG.)*
